...
def gamornet_train_keras(training_imgs,training_labels,validation_imgs,validation_labels,input_shape,files_path="./",epochs=100,checkpoint_freq=0,batch_size=64,lr=0.0001,momentum=0.9,decay=0.0,nesterov=False,loss='categorical_crossentropy',load_model=False,load_model_path="./",save_model=True,verbose=1):
	...
	if checkpoint_freq != 0:
		checkpoint = ModelCheckpoint(files_path + 'model_{epoch:02d}.hdf5', monitor='val_loss', verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=checkpoint_freq)
		callbacks_list.append(checkpoint)

	csv_logger = CSVLogger(files_path + "metrics.csv", separator=',', append=False)
	...
	if save_model == True:
		model.save(files_path + "trained_model.hdf5")
